...
if(path === "leaderboard"){
    ...
    const whichPlace = (idx) => {
        ...
        return wire()`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" fill="${colorMap[idx]}" class="medal">
        <path
    ...
    let lbData = {};
    (async () => {
        const res = await fetch('./leaders').catch(e => {
            console.error(e)
            Alerts.showAlert("danger", "Unable to get leaders")
        });
        lbData = await res.json();
        lbData.sort((a, b) => b.score - a.score)
        bind(lb)`${lbData.map((item, idx) => {
            return wire()`<div class="row row-border lb-row">
                    <div class="col-2">
                        ${whichPlace(idx)}
                    </div>
                    <div class="col-3">
